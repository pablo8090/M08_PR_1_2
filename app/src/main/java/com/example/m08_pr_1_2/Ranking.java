...
public class Ranking extends AppCompatActivity {
    ...
    private TextView generateTV (String text, int color, TableRow.LayoutParams textViewParams)
    {
    ...
    private TableRow generateRow (String[] items, TableRow.LayoutParams textViewParams, TableRow.LayoutParams tableRowParams) {
        ...
        {
            TextView tv = generateTV(items[i], Color.BLACK, textViewParams);
            tr.addView(tv);
        }
        return tr;
    }
    protected void onCreate(Bundle savedInstanceState) {
        ...
        headers[1] = "INTENTS";
        headers[2] = "TEMPS (s)";
        tbl.addView(generateRow(headers, textViewParams, tableRowParams));

        // Setting all results rows
        for (Result r : results)
        {
            String[] items = r.getData();
            tbl.addView(generateRow(items, textViewParams, tableRowParams));
        }
